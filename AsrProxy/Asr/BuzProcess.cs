    ...
    internal class BuzProcess : IAsr, ITranslate
    {
        ...
        public bool AudioRecog(byte[] audioData, LanguageType languageType, out string recogResult)
        {
            ...
            while (true)
            {
                ...
                if ((DateTime.Now - startTime).TotalMilliseconds >= 30 * 1000)
                    break;  // 超时时间设置为 30 s，需多调试，确定一个最佳的超时时间
        ...
        public bool Trans(string text, LanguageType from, out string result, LanguageType to = LanguageType.Mandarin)
        {
            ...
            while (true)
            {
                ...
                if ((DateTime.Now - startTime).TotalMilliseconds >= 10 * 1000)  // 超时时间 10s
                    break;
        ...
        public List<Language> GetTransLanguages()
        {
            ...
            while (true)
            {
                ...
                if ((DateTime.Now - startTime).TotalMilliseconds >= 5 * 1000)  // 超时时间 5s
                    break;
            ...
            if (_0x0606flag)
            {
                return _languageList;    // 可识别的语种列表和可翻译的语种列表共用一个变量
            }
            else
            {
                return new List<Language>();
        ...
        // 数据到达
        private void _client_OnData(object sender, Entity.RecvDataEventArgs e)
        {
            lock (_queueLock)
            {
