...
namespace AsrProxy
{
    ...
    internal class Client : IDisposable
    {
        ...
        private void LoadInfoFromConfig()
        {
            try
            {
                string configPath = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "AsrProxy.xml");
                XmlDocument doc = new XmlDocument();
                doc.Load(configPath);

                XmlNode node = doc.SelectSingleNode("./configuration/proxy/ip");
                _ip = node.InnerXml;
                node = doc.SelectSingleNode("./configuration/proxy/port");
