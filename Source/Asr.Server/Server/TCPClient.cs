...
namespace AsrServer
{
    ...
    internal class TCPClient : IDisposable
    {
        ...
        /// 数据缓存队列
        /// </summary>
        private Queue<RecvData> _dataQueue = new Queue<RecvData>();
        /// <summary>
        /// 数据解析类
        /// </summary>
        ...
        public TCPClient(Guid guid, Socket socket)
        {
            _id = guid;
            _socket = socket;
            _analyzer = new DataAnalyzer();
            RemoteIP = ((IPEndPoint)socket.RemoteEndPoint).Address.ToString();
            Utils.ShowInfo(this, string.Format("[Server] 客户端 {0} 连接成功。", RemoteIP));
            ...
            while (true)
            {
                try
                {
                    ...
                    if (count > 0)
                    {
                        ...
                        lock (_queueLock)
                        {
                            _dataQueue.Enqueue(new AsrServer.RecvData() { Data = data });
                        }
                        _semaphore.Release();
                    }
                }
        ...
        private void DealData()
        {
            while (true)
            {
                ...
                lock (_queueLock)
                {
                    // 此处可能引发队列为空的异常：按正常流程是不可能的，只是在停止时，有可能先清空了_dataQueue，然后再停止线程，所以引发异常
                    // 解决办法：在调用Clear()方法时也加锁，且在停止线程之后再调用
        ...
        private void AnalyzeData(byte[] data)
        {
            ...
            switch (dataType)
            {
                ...
                // 未处理的数据类型
                default:
                    Utils.ShowInfo(this, string.Format("[{0}] 收到 {1} 数据。但未做处理。", RemoteIP, string.Format("{0:X}", dataType)));
                    break;
            }
        }
        ...
        private void Deal0x0501(byte[] data)
        {
            ...
            if (ret == false)
            {
                string msg = "[Server] 解析 0x0501 数据（语音识别）失败。";
                Utils.ShowInfo(this, msg);
                return;
            }

            string recgResult;
            ret = Asr.Core.Asr.GetInstance().AudioRecog(audioData, languageType, out recgResult);
        ...
        {
            List<Language> languageList = Asr.Core.Asr.GetInstance().GetLanguageList();
        ...
        private void Deal0x0503(byte[] data)
        {
            ...
            if (ret == false)
            {
                Utils.ShowInfo(this, "[Server] 解析 0x0503 数据失败。");
                return;
            }

            LanguageType type = Asr.Core.Asr.GetInstance().Text2LanguageType(text);

            byte[] sendData = _analyzer.Combine0x0603(type);
            Send(sendData, "0x0603 （LanguageType映射）");
        }
        ...
        private void Deal0x0504(byte[] data)
        {
            ...
            if (ret == false)
            {
                Utils.ShowInfo(this, "[Server] 解析 0x0504 数据失败。");
                return;
            }

            LanguageType type = Asr.Core.Asr.GetInstance().Name2LanguageType(name);
        ...
        private void Deal0x0505(byte[] data)
        {
            ...
            if (ret == false)
            {
                Utils.ShowInfo(this, "[Server] 解析 0x0505 数据失败。");
                return;
            }

            string transResult;
            ret = Translate.GetInstance().Trans(text, from, out transResult, to);
        ...
        {
            List<Language> languageList = Translate.GetInstance().GetTransLanguages();
            if (languageList != null && languageList.Count > 0)
            {
        ...
        private void KeepAlive()
        {
            ...
            catch
            {
                ...
                Utils.ShowInfo(this, string.Format("[Server] 客户端 {0} 断开连接。", RemoteIP));
            }
        }
    }
}
